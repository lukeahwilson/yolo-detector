...
for i, batch in enumerate(im_batches):
    ...
    if CUDA: # Need to confirm whether necessary or not
...
# Time to start drawing boxes on images
im_dim_list = torch.index_select(im_dim_list, 0, output[:,0].long())
scaling_factor = torch.min(inp_dim/im_dim_list,1)[0].view(-1,1)

# Transform bounding boxes to match the size of the original images and not the transformed and processed image
output[:,[1,3]] -= (inp_dim - scaling_factor*im_dim_list[:,0].view(-1,1))/2
output[:,[2,4]] -= (inp_dim - scaling_factor*im_dim_list[:,1].view(-1,1))/2

output[:,1:5] /= scaling_factor

# Clip bounding boxes to fit in the original image size
